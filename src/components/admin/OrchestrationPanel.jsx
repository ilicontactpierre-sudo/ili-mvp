...
function buildExportPrompt(segments, soundLibrary) {
  ...
  lines.push('')
  lines.push('Sur tout bloc `layer: "music"` où une musique composée sur mesure servirait mieux la scène qu\'un son de bibliothèque, ajoute un champ optionnel `sunoPrompt` : une chaîne prête à copier-coller dans Suno.')
  lines.push('')
  lines.push('- Structure en 5-8 descripteurs séparés par des virgules, dans cet ordre de priorité : genre/sous-genre d\'abord, puis instrument(s) principal(aux), mood, tempo (BPM si pertinent), référence d\'époque/production.')
  lines.push('- Longueur cible : 150-300 caractères.')
  lines.push('- Sois précis comme un vrai brief d\'artiste : pas "piano triste" mais "piano solo mélancolique, notes espacées, léger bruit de mécanique, enregistrement intime". La précision vient de la palette sonore de l\'histoire, pas d\'une formule toute faite.')
  lines.push('- Exclusions artistiques uniquement si elles découlent d\'une vraie intention pour CETTE œuvre (ex : histoire folklorique → exclure explicitement structure et sonorités pop). Jamais d\'exclusion par défaut.')
  lines.push('- Le champ `note` du bloc explique l\'intention dramatique de la musique, séparément du prompt Suno lui-même.')
  lines.push('')
  lines.push('---')
  lines.push('')
  ...
  lines.push('')
  lines.push('La réponse contient, dans l\'ordre : le bloc `<script>`, PUIS un premier tableau JSON (pauses proposées, `[]` si aucune), PUIS un second tableau JSON (sons).')
  lines.push('')
  lines.push('**Tableau 1 — pauses proposées :**')
  lines.push('```json')
  lines.push('[')
  lines.push('  {')
  ...
  lines.push('    "note": "Pas sur gravier, accompagne toute la marche. Présence continue, pas ponctuelle."')
  lines.push('  },')
  lines.push('  {')
  ...
  lines.push('    "note": "Thème principal d\'Elsa. Second plan dialogues, premier plan descriptions."')
  lines.push('  }')
  lines.push(']')
  lines.push('```')
  lines.push('')
