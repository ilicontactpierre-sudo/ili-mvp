...
app.post('/api/publish', express.json(), async (req, res) => {
  ...
  if (!slug || !storyData || !storyData.title) {
    return res.status(400).json({ error: 'Données manquantes : slug et storyData requis' })
  }
  ...
  try {
    ...
    await writeGitHubFile(storyPath, storyData, `story: publish "${storyData.title}"`, storySha)
    ...
    const entry = { id: slug, title: storyData.title, author: storyData.author }
    ...
    await writeGitHubFile(indexPath, index, `index: ${existingPosition >= 0 ? 'update' : 'add'} "${storyData.title}"`, indexSha)
    return res.status(200).json({ success: true, isUpdate: existingPosition >= 0, message: `"${storyData.title}" publiée avec succès.` })
  } catch (error) {
